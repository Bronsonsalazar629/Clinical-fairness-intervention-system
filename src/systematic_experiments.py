...
class SystematicBenchmarkExperiments:
    ...
    def _generate_synthetic_compas(self) -> Tuple[pd.DataFrame, str, str]:
        ...
        race_binary = np.random.binomial(1, 0.51, n_samples)
        age = np.random.normal(34, 10, n_samples).clip(18, 70)
        sex = np.random.binomial(1, 0.81, n_samples)
        ...
        c_charge_degree = np.random.binomial(1, 0.46, n_samples)
    ...
    def _perform_statistical_tests(
        ...
        for method in methods:
            ...
            t_stat_fnr, p_val_fnr = stats.ttest_rel(baseline_fnr, method_fnr)
